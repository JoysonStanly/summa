...
import { MySidebar } from '../components/navigation';
import { ProblemStatement, CodeEditor, TestCases, Submissions } from '../components/problem';
...
import { Discussion, NotesEditor } from '../components/ui';
...
const ProblemPage = () => {
  ...
  const initialLang = getLanguageForProblem(problemId) || 'java';
  ...
  const { addCompletedProblem, updateStreak } = useUserStore();
  ...
  useEffect(() => {
    if (problem) setCurrentProblem(problemId);
  }, [problemId, problem, setCurrentProblem]);
  ...
  const handleLanguageChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setLanguage(langKey);
    setLanguageForProblem(problemId, langKey);
  };

  const defaultCode = problem.defaultCode[language as keyof typeof problem.defaultCode] || problem.defaultCode.typescript;
          ...
          <div className="px-4 sm:px-6 lg:pr-2">
            <div className="flex items-center justify-between h-16">
              <div className="block md:hidden">
                <img src="/static/media/TufPlusLight.041fc694d612b3fbaaa0.png" alt="TufPlusLogo" className="w-20 h-auto" />
              </div>
        ...
        {isLg ? (
          <PanelGroup direction="horizontal" className="flex-1">
            ...
            <Panel 
              ...
              minSize={15} 
              maxSize={70}
              className="flex flex-col border-r border-[#1f1f1f] h-full bg-[#0e0e0e] overflow-hidden"
            >
              {/* Header */}
            ...
            {/* Resizable Handle */}
            <PanelResizeHandle className="w-[10px] relative flex items-center justify-center bg-[#0e0e0e] hover:bg-[#1a1a1a] transition-colors duration-150 group">
              <div className="absolute w-[2px] h-full bg-[#1a1a1a] group-hover:bg-[#2a2a2a] transition-colors duration-150" />
              <div className="absolute w-3 h-3 bg-[#FF6D00] rounded-full border border-[#0e0e0e] shadow-[0_0_6px_rgba(255,109,0,0.6)] group-hover:scale-110 transition-transform duration-150" />
            </PanelResizeHandle>
            ...
            <Panel 
              ...
              minSize={30} 
              maxSize={70}
              className="flex flex-col h-full bg-[#0e0e0e] overflow-hidden relative"
            >
              {/* Editor Header */}
              <div className="flex items-center justify-between px-5 py-3 border-b border-[#1f1f1f] sticky top-0 bg-[#0b0b0b] z-20">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    title="Reset Code"
                    onClick={() => {
                      const fresh = problem.defaultCode[language as keyof typeof problem.defaultCode] || '';
                      setEditorKey(k => k + 1);
                      updateEditorContent(problemId, fresh);
                      setToastVariant('info');
                      setToastMsg('Editor reset to starter code');
                    }}
                  >
                    <RotateCcw size={16} />
                  </button>
                  <div className="w-px h-5 bg-[#1f1f1f]" />
                  <button className="text-gray-400 hover:text-white" title="AI Assistant">
                    <BrainCircuit size={16} />
                  </button>
                  <button
                    className="h-8 px-4 rounded-md text-sm font-medium bg-[#1a1a1a] hover:bg-[#2a2a2a] text-white flex items-center gap-1"
                    onClick={async () => {
                      const result = await submissionService.runLocally(problemId, '', language);
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      setToastMsg(result.message || 'Submitted');
                    }}
                  >
                    Submit
                  </button>
              ...
              <div className="flex-1 overflow-hidden">
                <CodeEditor
                  ...
                  onChange={value => {
                    if (value !== undefined) updateEditorContent(problemId, value);
                  }}
                />
              </div>
...
const EditorialView = ({ problem }: EditorialViewProps) => {
...
• In case the target value is not found, return -1 marking that the target element is missing.
  ...
  return (
    <Editorial
      ...
      // Solutions in different languages (using problem's default code)
      solutions={problem.defaultCode}
