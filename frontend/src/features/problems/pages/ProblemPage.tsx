                ...
                <div className="relative flex-1 overflow-hidden">
                  ...
                  >
                            ...
                            <div className="absolute top-full mt-1 left-0 bg-[#1a1a1a] border border-[#2a2a2a] rounded shadow-lg z-50 py-1 min-w-[60px]">
                              {[12, 13, 14, 15, 16, 17, 18, 19, 20].map(size => (
                                <button
                                  ...
                                  className={`w-full px-3 py-1.5 text-xs text-left hover:bg-[#2a2a2a] transition-colors ${editorConfig.fontSize === size ? 'text-orange-400 bg-[#2a2a2a]' : 'text-gray-300'
                                    }`}
                                >
                                  {size}
                                </button>
                    ...
                    >
                      ...
                      <div
                        className={`absolute w-full transition-all duration-200 ${isVerticalDragging
                      ...
                      <div
                        className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full transition-all duration-150 ${isVerticalDragging
                  ...
                  <div
                    className={`absolute top-0 left-0 h-full flex flex-col bg-[#0e0e0e] border-r border-[#1f1f1f] z-30 ${isDraggingState
                    ...
                    {leftPanelWidth < 100 ? (
                      <div className="flex flex-col items-center py-4 gap-4 h-full bg-[#0e0e0e]">
                        ...
                        {[
                          ...
                          { id: 'think-first', icon: Lightbulb, label: 'Think First' },
                          ...
                          <button
                            key={tab.id}
                            className={`flex flex-col items-center gap-3 py-3 transition-colors group ${activeTab === tab.id
                              ? 'text-[#FF6D00]'
                      ...
                      <>
                        ...
                        <div className="flex border-b border-[#1f1f1f] px-4 sticky top-0 bg-[#0e0e0e] z-10 overflow-x-hidden">
                          {[
                            ...
                            <button
                              key={tab.id}
                              className={`flex items-center gap-2 px-4 py-3 text-sm font-medium transition-colors relative whitespace-nowrap ${activeTab === tab.id
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar">
                          {isSwitchingProblem ? (
                            ...
                            <>
                              ...
                              {activeTab === 'think-first' && <ThinkFirstView problem={problem} />}
                  ...
                  <div
                    className={`absolute top-0 h-full flex items-center justify-center cursor-col-resize z-40 group $${isDraggingState ? 'w-1' : 'w-3 hover:w-4'
                      }`}
                    style={{
                      left: `${leftPanelWidth}px`,
                  ...
                  >
                    {/* Drag line */}
                    <div className={`absolute h-full transition-all duration-200 ${isDraggingState
                    ...
                    <div className={`absolute top-1/2 -translate-y-1/2 rounded-full bg-[#FF6D00] border-2 border-[#0e0e0e] cursor-col-resize transition-all duration-150 ${isDraggingState
                ...
                <div className="flex flex-col flex-1 h-full overflow-hidden">
                  ...
                  <div className="flex border-b border-[#1f1f1f] px-0 sticky top-0 bg-[#0e0e0e] z-20 overflow-x-auto scrollbar-hide">
                    {[
                      ...
                      <button
                        key={tab.id}
                        className={`flex items-center gap-2 px-3 sm:px-4 py-3 text-xs sm:text-sm font-medium transition-colors relative whitespace-nowrap ${activeTab === tab.id
                          ? 'text-white border-b-2 border-[#FF6D00]'
                          : 'text-gray-400 hover:text-gray-300'
                          }`}
                        onClick={() => navigate(`/dsa/${topicId}/${subtopicId || actualProblemId}/${actualProblemId}?tab=${tab.id}`)}
...
// 📌 Think First View Component
import ThinkFirstInteractive from '../components/think-first/ThinkFirstInteractive';
import { linearSearchQuestions } from '../data/thinkFirst/linearSearchQuestions';

type ThinkFirstViewProps = {
  problem: any;
};

const ThinkFirstView = ({ problem }: ThinkFirstViewProps) => {
  // Map problem IDs to their Think First questions
  // For now, we'll use Linear Search questions as the default
  // In the future, this can be expanded to support more problems
  const getQuestionsForProblem = (problemId: string) => {
    // You can add more mappings here as you create more Think First content
    const questionMap: Record<string, any> = {
      'linear-search': linearSearchQuestions,
      // Add more problems here:
      // 'binary-search': binarySearchQuestions,
      // 'bubble-sort': bubbleSortQuestions,
    };

    return questionMap[problemId] || linearSearchQuestions; // Default to linear search
  };

  const questions = getQuestionsForProblem(problem.slug || problem.id);

  return <ThinkFirstInteractive questions={questions} problemTitle={problem.title} />;
};
