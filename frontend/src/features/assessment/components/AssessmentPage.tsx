...
const AssessmentPage: React.FC<AssessmentPageProps> = ({ 
  ...
  // Fetch quizzes from API (optional, fallback to static data)
  useEffect(() => {
    const fetchQuizzes = async () => {
      try {
        setLoading(true);
        const quizzes = await quizService.getQuizzes(quizType);
        setApiQuizzes(quizzes);
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      } catch (error) {
  ...
  // Redirect to first category/subcategory if none provided
  useEffect(() => {
    if (!categoryId || !subCategoryId) {
      const firstCategory = categories[0];
      const firstSubCategory = firstCategory?.subCategories[0];
      if (firstCategory && firstSubCategory) {
        navigate(`${basePath}/${firstCategory.id}/${firstSubCategory.id}`, { replace: true });
      }
    }
  }, [categoryId, subCategoryId, navigate, categories, basePath]);
  ...
  const handleSelectAnswer = (questionId: number, optionId: string) => {
    if (revealedAnswers[questionId]) return; // Prevent changing answer after reveal
                ...
                <div className="p-5">
                  ...
                  {revealedAnswers[question.id] && (
                    <div className="mt-5 pt-5 border-t border-[#676A6D]/20 pl-11">
                      ...
                      {showSolution[question.id] && question.explanation && (
                        <div className="mt-4">
