...
const AptitudePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchQuizzes = async () => {
      try {
        setLoading(true);
        const quizzes = await quizService.getQuizzes('aptitude');
  ...
  useEffect(() => {
    if (!categoryId || !subCategoryId) {
      const firstCategory = aptitudeCategories[0];
      const firstSubCategory = firstCategory.subCategories[0];
      navigate(`/quiz/${firstCategory.id}/${firstSubCategory.id}`, { replace: true });
    }
  }, [categoryId, subCategoryId, navigate]);
  
  // Find current category and subcategory
  const currentCategory = aptitudeCategories.find(cat => cat.id === categoryId) || aptitudeCategories[0];
