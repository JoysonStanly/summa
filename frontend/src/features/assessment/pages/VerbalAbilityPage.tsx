...
import { verbalAbilityCategories } from '../data/verbalAbilityData';
...
const VerbalAbilityPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchQuizzes = async () => {
      try {
        await quizService.getQuizzes('verbal-ability');
      } catch (error) {
  ...
  const currentCategory = verbalAbilityCategories.find(cat => cat.id === categoryId) || verbalAbilityCategories[0];
    ...
    return (
      <div className="flex h-screen bg-background">
        <UnifiedSidebar
          ...
          searchPlaceholder="Search Verbal Topics..."
