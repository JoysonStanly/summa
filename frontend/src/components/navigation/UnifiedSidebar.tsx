...
import { User, Settings, Bug, ListChecks, Bell, BellOff, LogOut, ChevronRight, FileText, Navigation, FilePen, Home, type LucideIcon } from 'lucide-react';
                  ...
                  {expandedCategories[category.id] && subItems.length > 0 && (
                    <div className="mt-4 ml-1 space-y-4">
                      {subItems.map((subItem) => (
                        <Link
                          key={subItem.id}
                          to={`${basePath}/${category.id}/${subItem.id}`}
                          className={`flex cursor-pointer py-2 pl-3 ${
                            isActive(category.id, subItem.id)
                              ? theme === 'dark'
                        ...
                        >
                          <span className="mr-2">
                            <CheckIcon isActive={isActive(category.id, subItem.id)} />
                          </span>
